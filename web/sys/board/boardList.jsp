			...
			<c:forEach items="${requestScope.list}" var="table" varStatus="vs">
				<tr class="TableDetail1">
					...
					<td>
						<c:choose>
							<c:when test="${table.tableStatus == 1}">
								<a href="${pageContext.request.contextPath}/table?method=update&id=${table.id}" class="FunctionButton">退桌</a>
							</c:when>
							<c:otherwise>
								<a href="${pageContext.request.contextPath }/table?method=update&id=${table.id}" class="FunctionButton">预定</a>
							</c:otherwise>
						</c:choose>
